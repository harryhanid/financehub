...
**Date / Tanggal:** 2026-06-22
**Version / Versi:** 1.4
**Status:** Updated — Comprehensive Refresh: Masterplan, Data Architecture, Dual-Company Balance
...
| 1.3 | 2026-06-10 | Tambah: ETF Payment Application (AGRI/APP/LAND/SETF), PAM Input Integration (unified save panel), PA Picker sub-row, Editable PAM No. (format validation + collision check), PAM↔PA workflow fix, Status Standardization (open/on_process/complete), Global Confirmation Modal |
| 1.4 | 2026-06-22 | Comprehensive Refresh: Masterplan statuses, rename LAND+ENERGY PA, remove test coverage, dual-company narrative, full data architecture, pillar taxonomy |
...
| Fase | Sistem | Status |
|---|---|---|
| **F1** — Budget & Master Data | Google Sheets / SQL | TBA (Concept) |
| **F2** — Purchasing (Web Portal) | **PR Portal** (Google Apps Script) | Live Trial |
...
- **ETF Payment Application** diproses manual — tidak ada sistem untuk tracking 7 SLA dates per PA, tidak ada link otomatis ke PAM number.
...
8. **ETF Payment Application (AGRI/APP/LAND/SETF)** — modul khusus untuk mencatat, tracking, dan mengeksekusi Payment Application beasiswa ETF dengan 7 SLA dates per PA dan 4 tipe PAM.
9. **PAM Input Integration** — Input panel terpadu di Payment Memo yang langsung membaca PA open (AGRI/APP/LAND/SETF), menghasilkan PAM number otomatis, dan mengupdate status PA secara atomik.
...
| Lihat daftar PA (AGRI/APP/LAND/SETF) | ✅ | ✅ | ✅ |
| Buat PA baru | ✅ | ✅ | ✅ |
...
### 4.3 Alur ETF Payment Application (AGRI/APP/LAND/SETF)

```
Dashboard ETF
    → Menu: ETF Payment Application
    → Pilih tab: [AGRI] | [APP] | [LAND] | [SETF]
...
Dashboard [ETF]
    ...
    → Pilih Tipe PAM: [AGRI ▾] | [APP] | [LAND] | [SETF]
...
Dashboard [SMT | ETF]
    ...
    → Tab: Daftar PAM
        ...
        → Filter source (Semua Tipe / AGRI / APP / LAND / SETF)
...
│       │   ├── routes.py           ← HTML routes: /etf-payment-application (AGRI/APP/LAND/SETF)
...
│   │   └── index.html
│   └── etf_payment_application/
│       └── index.html              ← tabs: AGRI / APP / LAND / SETF
├── static/
...
|---|---|---|
| `open` | Baru dibuat, belum diproses | payment_beasiswa, payment_memo, pam_records, payment_application, etf_pa, app_pa, land_pa, setf_pa |
...
| Payment Approval Memo | ✅ Implemented | PAM Records, Draft Memo, Days of PAM, GL Account, Input PA (AGRI/APP/LAND/SETF), PDF/Excel export |
| Payment Application | ✅ Implemented | Monitoring status pengajuan & TAT |
| ETF Payment Application | ✅ Implemented | Tracking PA per tipe (AGRI/APP/LAND/SETF) dengan 7 SLA dates; auto-link ke PAM |
...
| Tab | Fungsi |
|---|---|
| **Daftar PAM** | Tabel semua PAM records, filter per tipe (AGRI/APP/LAND/SETF), isi/update GL Account + Catatan Payment |
...
| **Input PA** | Panel input terpadu untuk AGRI/APP/LAND/SETF: pilih tipe → siswa → PA lines → simpan atomik |
...
1. Jalur A — Via Input Payment Beasiswa (tab Beasiswa):
   ...
   b. Service layer (atomik):
      ...
      - Update nomor_pam ke etf_pa/app_pa/land_pa yang terhubung
   c. PAM muncul di tab "Daftar PAM"

2. Jalur B — Via Input PA (tab Payment Memo → Input PA):
   a. User pilih Tipe PAM (AGRI/APP/LAND/SETF)
...
- Mendukung **4 tipe PA**: AGRI, APP, LAND, SETF — masing-masing dengan tabel DB sendiri
...
| LAND | `land_pa` | `land_pa_lines` | LAND |
...
_TAB_CFG = {
    ...
    "sml":   ("land_pa",  "land_pa_lines",  "LAND",  "LAND"),
...
User set Tgl Bayar → cascade: status → 'complete', tanggal_bayar diisi
    (cascade juga ke etf_pa, app_pa, land_pa via set_memo_tanggal_bayar)

Cancel PAM → PA dikembalikan ke 'open', nomor_pam → NULL
    (cascade ke etf_pa, app_pa, land_pa via cancel_pam_record)
```
...
- Tab per tipe: `[AGRI] [APP] [LAND] [SETF]`
...
-- ETF Payment Application (LAND) — identik
CREATE TABLE IF NOT EXISTS land_pa  ( /* schema identik etf_pa  */ );
CREATE TABLE IF NOT EXISTS land_pa_lines  ( /* schema identik etf_pa_lines  */ );
...
| Method | Path | Keterangan |
|---|---|---|
| GET | `/etf-payment-application` | Halaman daftar PA (tab AGRI/APP/LAND/SETF) |
...
| **ETF Payment Application (APP/LAND)** | ✅ | Extend ke app_pa/land_pa via `_TAB_CFG`; semua endpoint generik |
| **ETF Payment Application (SETF)** | ✅ | Extend ke setf_pa/setf_pa_lines; identik schema |
...
| **PAM ↔ PA Workflow Fix** | ✅ | nomor_pam propagasi saat save; cascade paid-date ke app_pa + land_pa; status filter hanya `open` |
...
| **SLA Chip Selection (AGRI/APP/LAND)** | ✅ | Chip selection visibility di ETF PA tab |
...
| **SETF PA halaman UI** | Input/kelola setf_pa records | 🔜 |
...
*Document updated: 2026-06-10 | Finance Hub PRD v1.3 — Post Fase 1 full implementation + ETF Payment Application + PAM Automation*
